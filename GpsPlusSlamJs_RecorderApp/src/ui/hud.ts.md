...
| `updateRefPointHint(nearby?)`              | `({displayName, isNeighborCell}?) => void` | D3: inline proximity hint (`#ref-point-hint`) that makes the name relabel read as a **location confirmation**. `undefined` → hidden. Same cell → "You're at '\<name>' — tap 📍 to re-observe it." Neighbour cell → adds the "or ➕ to mark a new point here" option. Display-only (no behaviour/name-management change). Lazily looked up; no-op if absent. Reset to hidden in `showArReadyControls`.                                                                                                                                                                                             |
...
| `setFolderImportExpanded(expanded, hint?)` | `(boolean, string?) => void`               | Expand/collapse the optional `#folder-import-section` `<details>` and show/clear a one-line hint above the folder button. The hint is **gated on `expanded`** (it explains why the section auto-expanded; collapsing always clears it). D5: collapsed by default; auto-expanded by `folder-manager` when the chosen scenario has no OPFS ref points, and by `main.ts` in replay mode. Degrades gracefully when the elements are absent.                                                                                                                                                           |
| `setFolderImportProgress(state)`           | `(FolderImportProgressState) => void`      | Drive the determinate folder-import progress bar + text label (`#folder-import-progress*`, D2 of the 2026-07-05 folder-import feedback). `{kind:'progress',done,total}` → "Recovering reference points… x / y recordings" with proportional bar width (hidden when `total<=0`); `{kind:'done',refPointsWritten,zipFilesTotal}` → durable ✓ summary at 100% that lingers 4 s then hides (async-UX rule); `null` → immediate hide + width reset (failure/abort — errors surface via toast/error channel), also cancelling a pending linger timer. Degrades gracefully when the elements are absent. |
