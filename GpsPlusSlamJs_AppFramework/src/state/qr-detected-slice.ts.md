...
- **Derive-on-read selectors (decision D-A)**:
  ...
  - `selectDerivedQrPlacement(state, text, deps)` → `DerivedQrPlacement | null` (`{ pose, sizeM }`). Same derivation, but returns the solved pose AND the metric size that solved it in one history pass — what the WS-5 debug viz consumes (the cube needs the size). The recorder's `qr-debug-controller` calls this per marker on each store change.
