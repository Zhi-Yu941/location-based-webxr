...
| `clearRefPointsCacheForAllScenarios()` | `async () => ClearRefPointsCacheResult` | Maintenance action: removes the cached `refPoints/` directory from every scenario in OPFS so the next scenario load re-imports ref points from `*.zip` recordings. Errors per scenario are collected; missing caches are not errors. **Throws** if OPFS storage is unavailable (e.g. `initStorage()` not yet called) — silent zero-count would mask a real failure from the UI. |
| `ClearRefPointsCacheResult`            | interface                               | `{ scenariosCleared, scenariosScanned, errors[] }`                                                                                                                                                                                                                                                                                                                              |
