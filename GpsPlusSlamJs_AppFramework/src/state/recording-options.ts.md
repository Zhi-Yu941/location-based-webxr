...
| `validateOccupancyOptions(partial)`        | `Partial<OccupancyOptions>`        | `OccupancyOptions`        | Clamps `cellSizeM`; rounds + clamps `minConfidence` (1–10); coerces `persistentOcclusion`/`liveOcclusion`/`occluderDebugViz` to boolean (non-boolean → default false); enum-validates `occluderMeshMode` against `OCCLUDER_MESH_MODES` (unknown → default `'smooth'`); **migrates** a legacy `occlusionMeshEnabled` (absent new field → maps `true`→`persistentOcclusion`); rejects NaN/Infinity to default |
